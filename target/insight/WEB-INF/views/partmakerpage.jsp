...
<body bgcolor="#f4f6f8"
	...
	<div class="row">
		<div class="row">
			<form class="col s12" role="form" method="post" id="partmakerform"
				action="/insight/parts/partmaker">
				<div class="row">
					<i class="small mdi-communication-vpn-key">Part Maker</i>
				</div>
				<div class="row">
					<input type="hidden" id="partmakermode" name="partmakermode"
						value="">
				</div>
				<div class="card white">
					<div class="card-content">
						<div class="row">
							<div class="col s2">
								<label><strong>Select Make</strong></label> <select
									id="makeselected" name="makeselected" class="browser-default">
							...
							<div class="col s2">
								<span>
									<button class="btn waves-effect waves-light" type="submit"
										id="partnocheck" name="partnocheck" value="partnocheck">Get
										Next #</button>
								</span>
							</div>
							<div class="col s1">
								<c:if test="${not empty nextpartnumber}">
									<h3 class="header">${nextpartnumber}</h3>
								</c:if>
							</div>
						</div>
					</div>
	...
	<script>
		$(document).ready(function() {
			...
			$("#partnocheck").click(function() {

				$('input[name=partmakermode]').val('partnocheck');
				 $("#partmakerform").attr("action", "/insight/parts/" + "partmaker");
				$("#partmakerform").submit();

			});

			$("#partmakerform").submit(function() {
				var isFormValid = true;
				if ($('#partmakermode').val() == 'partnocheck') {
					if ($('#subcategoryselected').val() == '') {
						alert('subcategory cannot be blank');
						isFormValid = false;
					}
