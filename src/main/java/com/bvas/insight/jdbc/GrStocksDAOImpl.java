...
@Repository
public class GrStocksDAOImpl implements BaseDAOImpl {

	private static final Logger LOGGER = LoggerFactory.getLogger(GrStocksDAOImpl.class);

	private DataSource grdataSource;

	public Boolean checkStockData(String partno) throws ConnectException {

		try {
			JdbcTemplate jdbcTemplate = new JdbcTemplate(grdataSource);
	...
	public List<InvCategoryBySalesAnalysis> getInventoryBySubcategory(String analyticsfromdate, String analyticstodate,
		...
		try {
			...
			switch (dataSource) {
			case "GR":
				jdbcTemplate = new JdbcTemplate(grdataSource);
				break;
			}
	...
	public StockCheck getStock(String partno) {
		...
		try {
			JdbcTemplate jdbcTemplate = new JdbcTemplate(grdataSource);
			String SQL = "SELECT p.PartNo,p.InterchangeNo, p.manufacturername, p.makemodelname, p.PartDescription, p.Location, p.UnitsInStock, p.UnitsOnOrder,p.reorderlevel,p.returncount,p.safetyquantity,p.ordertype,p.pricelock FROM parts p  WHERE p.PartNo = ?   ";
	...
	public void setGrdataSource(DataSource grdataSource) {

		this.grdataSource = grdataSource;
	}
}
