...
@Repository
public class ChStocksDAOImpl implements BaseDAOImpl {

	private static final Logger LOGGER = LoggerFactory.getLogger(ChStocksDAOImpl.class);

	private DataSource chdataSource;

	public void checkStockData(String partno) throws ConnectException {
	...
	public List<StockCheck> getAllStock(String partno) throws ConnectException {
		...
		try {
			JdbcTemplate jdbcTemplate = new JdbcTemplate(chdataSource);
			String SQL = "SELECT  p.PartNo,p.InterchangeNo, p.manufacturername, p.makemodelname, p.PartDescription, p.Location, p.UnitsInStock, p.returncount,p.UnitsOnOrder,p.reorderlevel,p.safetyquantity,p.ordertype,p.pricelock FROM parts p  WHERE ( p.PartNo = ?  or interchangeno = ? )  ";
	...
	public List<InvCategoryBySalesAnalysis> getInventoryBySubcategory(String analyticsfromdate, String analyticstodate,
		...
		try {
			...
			switch (dataSource) {
			case "CH":
				jdbcTemplate = new JdbcTemplate(chdataSource);
				break;
	...
	public StockCheck getStock(String partno) {
		...
		try {
			JdbcTemplate jdbcTemplate = new JdbcTemplate(chdataSource);
			String SQL = "SELECT p.PartNo,p.InterchangeNo, p.manufacturername, p.makemodelname, p.PartDescription, p.Location, p.UnitsInStock,p.returncount, p.UnitsOnOrder,p.reorderlevel,p.safetyquantity,p.ordertype,p.pricelock FROM parts p  WHERE p.PartNo = ?   ";
	...
	public void setChdataSource(DataSource chdataSource) {
